...
| **MySQL** | `ALTER TABLE table_name DROP PRIMARY KEY, ADD CONSTRAINT new_name PRIMARY KEY (columns)` | **Supported But Limited** | Cannot directly rename; must drop and recreate PK. Requires knowing column list. Risk of data inconsistency during operation |
...
#### MySQL/MariaDB
- **Syntax**: Cannot directly rename; must use:
  ```sql
  ALTER TABLE table_name 
  DROP PRIMARY KEY,
  ADD CONSTRAINT new_pk_name PRIMARY KEY (column_list);
  ```
- **Support**: Indirect support only
- **Considerations**: 
  - Requires knowledge of all columns in the primary key
  - Temporary removal of primary key constraint during operation
  - Cannot be done on tables with foreign key references in older versions
  - May require disabling foreign key checks
- **Auto-rename on table rename**: N/A - MySQL primary keys are often unnamed or have default names
...
- **Supported with Limitations**: 1 provider (MySQL - requires drop and recreate)
...
- Uses drop-and-recreate approach for MySQL with appropriate warnings
...
- For MySQL, provide helper methods that handle the drop-and-recreate pattern
...
- MySQL implementation is complex and potentially risky
