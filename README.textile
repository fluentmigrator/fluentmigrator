...
h2. Getting the example project running
...
The db:migrate commands can take an optional VERSION parameter so <pre><code>rake db:migrate VERSION=20090906205342</pre></code> 
will only migrate the first migration. db:rollback can take an optional STEPS parameter.
