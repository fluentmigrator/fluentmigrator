...
namespace FluentMigrator.Runner.Processors.SQLite
{
    ...
    public class SQLite2Processor : GenericProcessorBase
    {
        [CanBeNull]
        private readonly IServiceProvider _serviceProvider;

        public SQLite2Processor(
            [NotNull] SQLiteDbFactory factory,
            [NotNull] SQLite2Generator generator,
            [NotNull] ILogger<SQLite2Processor> logger,
        ...
        public override string DatabaseType => "SQLite2";
        ...
        {
            var dataSet = Read($"SELECT COUNT(*) AS Count FROM pragma_table_info('{tableName}') WHERE name IS '{columnName}'");
            var column = dataSet?.Tables[0]?.Rows[0]["Count"].ToString();
