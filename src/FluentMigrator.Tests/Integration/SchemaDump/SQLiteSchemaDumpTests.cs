...
namespace FluentMigrator.Tests.Integration.SchemaDump {
	...
	public class SQLiteSchemaDumpTests {
		...
		public void CanGenerateColumnDefinitionsFromTableDDL() {
			...
			//GetColumnDefinitionsForTableDDL( string tableDDL )
		...
		public void CanGetIndexeColumnsFromIndexDDLComplex() {
			//var indexDDL1 = @"CREATE UNIQUE INDEX [IDX_USERS_FullName] ON [Users]([FirstName]  ASC,[LastName]  ASC)";
		...
		public void CanReadBasicSchemaInfo() {
			...
			using ( var table = new SqliteTestTable( Processor, null, "id int" ) ) {
				try {
		...
		public void CanReadSchemaInfoWithIdentity() {
			using ( var table = new SqliteTestTable( Processor, null, "id INTEGER  PRIMARY KEY AUTOINCREMENT NOT NULL" ) ) {
				var defs = SchemaDumper.ReadDbSchema();
		...
		public void CanReadSchemaInfoWithNullable() {
			using ( var table = new SqliteTestTable( Processor, null, "id int NULL" ) ) {
				var defs = SchemaDumper.ReadDbSchema();
