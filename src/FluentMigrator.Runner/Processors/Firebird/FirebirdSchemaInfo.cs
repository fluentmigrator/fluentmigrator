...
#region FieldTypes by number
        ...
        private object GetDefaultValue(object val)
        {
            ...
            if (src.StartsWith("DEFAULT ", StringComparison.OrdinalIgnoreCase))
            {
                ...
                else if (value.Equals("current_timestamp", StringComparison.OrdinalIgnoreCase))
                {
                    return SystemMethods.CurrentDateTime;
                }
                else if (value.Equals("gen_uuid()", StringComparison.OrdinalIgnoreCase))
                {
