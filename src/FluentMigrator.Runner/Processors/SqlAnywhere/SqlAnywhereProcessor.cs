...
namespace FluentMigrator.Runner.Processors.SqlAnywhere
{
    public sealed class SqlAnywhereProcessor : GenericProcessorBase
    {
        ...
        private const string INDEX_EXISTS = "SELECT 1 WHERE EXISTS (SELECT i.* FROM sys.index AS i INNER JOIN sys.systable AS t ON t.table_id = i.table_id INNER JOIN sys.sysuserperm AS up ON up.user_id = t.creator WHERE i.index_name = '{0}' AND up.user_name = '{1}' AND t.table_name = '{2}')";
