...
namespace FluentMigrator.Runner.Processors.Oracle
{
    public class OracleProcessor : GenericProcessorBase
    {
        ...
        public override bool TableExists(string schemaName, string tableName)
        {
			...
			return Exists("SELECT 1 FROM ALL_TABLES WHERE upper(OWNER) = '{0}' AND upper(TABLE_NAME) = '{1}'", schemaName.ToUpper(), tableName.ToUpper());
        }
        ...
        public override bool ColumnExists(string schemaName, string tableName, string columnName)
        {
            ...
            if (string.IsNullOrEmpty(schemaName))
                return Exists("SELECT 1 FROM USER_TAB_COLUMNS WHERE upper(TABLE_NAME) = '{0}' AND upper(COLUMN_NAME) = '{1}'", tableName.ToUpper(), columnName.ToUpper());

			return Exists("SELECT 1 FROM ALL_TAB_COLUMNS WHERE upper(OWNER) = '{0}' AND upper(TABLE_NAME) = '{1}' AND upper(COLUMN_NAME) = '{2}'",
				schemaName.ToUpper(), tableName.ToUpper(), columnName.ToUpper());
        }
        ...
        public override bool ConstraintExists(string schemaName, string tableName, string constraintName)
        {
            ...
            if (String.IsNullOrEmpty(schemaName))
				return Exists("SELECT 1 FROM USER_CONSTRAINTS WHERE upper(CONSTRAINT_NAME) = '{0}'", constraintName.ToUpper());

			return Exists("SELECT 1 FROM ALL_CONSTRAINTS WHERE upper(OWNER) = '{0}' AND upper(CONSTRAINT_NAME) = '{1}'", schemaName.ToUpper(), constraintName.ToUpper());
        }
        ...
        public override bool IndexExists(string schemaName, string tableName, string indexName)
        {
            ...
            if (String.IsNullOrEmpty(schemaName))
				return Exists("SELECT 1 FROM USER_INDEXES WHERE upper(INDEX_NAME) = '{0}'", indexName.ToUpper());

			return Exists("SELECT 1 FROM ALL_INDEXES WHERE upper(OWNER) = '{0}' AND upper(INDEX_NAME) = '{1}'", schemaName.ToUpper(), indexName.ToUpper());
        }
