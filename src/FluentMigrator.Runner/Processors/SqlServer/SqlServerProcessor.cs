...
@"SELECT CASE WHEN EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '{0}' AND TABLE_NAME = '{1}')   
    THEN 1 
    ELSE 0 
END";

        private const string COLUMN_EXISTS =
@"SELECT CASE WHEN EXISTS (SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '{0}' AND TABLE_NAME = '{1}' AND COLUMN_NAME = '{2}')   
    THEN 1 
    ELSE 0 
END";

        private const string CONSTRAINT_EXISTS =
@"SELECT CASE WHEN EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE CONSTRAINT_CATALOG = DB_NAME() AND TABLE_SCHEMA = '{0}' AND TABLE_NAME = '{1}' AND CONSTRAINT_NAME = '{2}')   
    THEN 1 
    ELSE 0 
END";

        private const string INDEX_EXISTS =
@"SELECT CASE WHEN EXISTS (SELECT * FROM sys.indexes WHERE name = '{0}' and object_id=OBJECT_ID('{1}.{2}'))   
    THEN 1 
    ELSE 0 
END";

        private const string SEQUENCES_EXISTS =
@"SELECT CASE WHEN EXISTS (SELECT * FROM INFORMATION_SCHEMA.SEQUENCES WHERE SEQUENCE_SCHEMA = '{0}' AND SEQUENCE_NAME = '{1}' )   
    THEN 1 
    ELSE 0 
END";

        private const string DEFAULTVALUE_EXISTS =
@"SELECT CASE WHEN EXISTS (SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '{0}' AND TABLE_NAME = '{1}' AND COLUMN_NAME = '{2}' AND COLUMN_DEFAULT LIKE '{3}')   
    THEN 1 
    ELSE 0 
END";
