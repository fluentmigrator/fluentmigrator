...
{
    public abstract class PostgresBaseGenerator : GenericGenerator
    {
        public PostgresBaseGenerator(IColumn column, IQuoter quoter, IDescriptionGenerator generator) : base(column, quoter, generator)
        {
        }
        ...
        {
            var alterStatement = new StringBuilder();
            alterStatement.Append(String.Format("ALTER TABLE {0}.{1} {2}", Quoter.QuoteSchemaName(expression.SchemaName), Quoter.QuoteTableName(expression.TableName), ((PostgresBaseColumn)Column).GenerateAlterClauses(expression.Column)));
            var descriptionStatement = DescriptionGenerator.GenerateDescriptionStatement(expression);
            if (!string.IsNullOrEmpty(descriptionStatement))
            {
        ...
        {
            return String.Format("ALTER TABLE {0}.{1} ALTER {2} DROP DEFAULT, ALTER {2} {3}", Quoter.QuoteSchemaName(expression.SchemaName), Quoter.QuoteTableName(expression.TableName), Quoter.QuoteColumnName(expression.ColumnName), ((PostgresBaseColumn)Column).FormatAlterDefaultValue(expression.ColumnName, expression.DefaultValue));
        }
