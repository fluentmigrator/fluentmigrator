...
namespace FluentMigrator.Runner.Generators.Oracle
{
    ...
    {
        /// <summary>
        /// Taken from http://docs.oracle.com/cd/B19306_01/em.102/b40103/app_oracle_reserved_words.htm#CHDCGECI
        /// </summary>
        ...
        {
            return IsKeyword(columnName) ? base.QuoteColumnName(columnName) : columnName;
        }

        public override string QuoteConstraintName(string constraintName)
        {
            return IsKeyword(constraintName) ? base.QuoteColumnName(constraintName) : constraintName;
        }

        public override string QuoteIndexName(string indexName)
        {
            return IsKeyword(indexName) ? base.QuoteColumnName(indexName) : indexName;
        }

        public override string QuoteSchemaName(string schemaName)
        {
            return IsKeyword(schemaName) ? base.QuoteColumnName(schemaName) : schemaName;
        }

        public override string QuoteTableName(string tableName)
        {
            return IsKeyword(tableName) ? base.QuoteColumnName(tableName) : tableName;
        }

        public override string QuoteSequenceName(string sequenceName)
        {
            return IsKeyword(sequenceName) ? base.QuoteColumnName(sequenceName) : sequenceName;
        }
