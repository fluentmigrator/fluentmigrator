...
namespace FluentMigrator.Tests.Integration.ConnectionFactory.Postgres
{
    [TestFixture]
    [Category("ConnectionFactory")]
    [Category("Postgres")]
    public sealed class PostgresConnectionFactoryTests : ConnectionFactoryProviderTestBase
    {
        [Test]
        public void Postgres_WithConnectionFactory_RunsMigration()
        {
            var connectionString = GetRequiredEnvironmentVariableOrIgnore("FM_TEST_POSTGRES");
            ...
            try
            {
                using (var serviceProvider = CreateServiceProvider(runnerBuilder => runnerBuilder.AddPostgres(), _ =>
                {
